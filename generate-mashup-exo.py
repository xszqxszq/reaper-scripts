...
length={}
audio_rate=48000
audio_ch=2'''.format(video_w, video_h, fps, int(fps * audiofile.duration(mashup_audio)) + 100)
...
for index, track in tqdm.tqdm(enumerate(tracks)):
	...
	for item in items:
		...
		pos = int(float([i for i in item.children if isinstance(i, list) and i[0] == 'POSITION'][0][1]) * fps)
		start = int(float([i for i in item.children if isinstance(i, list) and i[0] == 'SOFFS'][0][1]) * now_fps)
		length = int(float([i for i in item.children if isinstance(i, list) and i[0] == 'LENGTH'][0][1]) * fps)
		speed = float([i for i in item.children if isinstance(i, list) and i[0] == 'PLAYRATE'][0][1]) * 100
		scale = item_w / width * 100

		x = x1 + ((index - 1) % w) * item_w
		y = y1 + ((index - 1) // w) * item_h
		exo += video.format(index - 1, pos, pos + length, start, speed, path, x, y, scale, index)
